...
const PricingPage = () => {
  ...
  return (
    <Box sx={{ bgcolor: '#FFFFFF', minHeight: '100vh', overflowX: 'hidden' }}>
      ...
      {/* 2. HERO SECTION */}
      <Box sx={{ bgcolor: '#F4F7FF', pt: { xs: 8, md: 12 }, pb: { xs: 15, md: 20 }, textAlign: 'center' }}>
        <Container maxWidth="md">
          <Typography variant="h3" sx={{ fontWeight: 900, color: '#1E293B', mb: 2, letterSpacing: '-0.02em', fontSize: { xs: '2rem', md: '3rem' } }}>
      ...
      <Container id="pricing-grid" maxWidth="lg" sx={{ mt: -12, pb: 10 }}>
        <Grid container sx={{ 
          ...
          {plans.map((plan, index) => {
            ...
            return (
              <Grid item key={plan.id} xs={12} md={4} sx={{ 
                ...
                <Box sx={{ p: { xs: 4, md: 5 }, height: '100%', display: 'flex', flexDirection: 'column' }}>
                  ...
                  <Box sx={{ mb: 4 }}>
                    <Stack direction="row" alignItems="baseline">
                      <Typography variant="h3" sx={{ fontWeight: 900, color: '#1E293B' }}>₹{pricePerMonth}</Typography>
                      <Typography variant="subtitle1" sx={{ color: '#64748B', ml: 1, fontWeight: 500 }}>/ month</Typography>
                    </Stack>
                    <Typography variant="caption" sx={{ color: '#94A3B8', fontWeight: 600 }}>
                      ₹{gstTotal} incl. 18% GST
                    </Typography>
                  </Box>
                  ...
                  <Button 
                    ...
                    disabled={isPaidActive || (status === 'PENDING' && !isThisPlanTier)}
                    sx={{ 
                  ...
                  >
                    {isPaidActive ? 'Current Plan' : isTrialActive ? 'On Trial (Upgrade Now)' : isPendingThisPlan ? 'Verifying...' : (isPlanExpired ? 'Renew Plan' : 'Buy Now')}
                  </Button>
                  ...
                  <List sx={{ p: 0, flexGrow: 1 }}>
                    {(plan.features || []).map((f, i) => (
                      <ListItem key={i} disableGutters sx={{ py: 0.6 }}>
                        ...
                        <ListItemText 
                          primary={f} 
                          primaryTypographyProps={{ variant: 'body2', color: '#475569', fontWeight: 500 }} 
                        />
                      </ListItem>
                    ))}
                  </List>
      ...
      {isTrialAvailable ? (
        ...
        <Container maxWidth="lg" sx={{ mb: 10 }}>
            {/* 6. OPTIONAL: Feedback if they already used their trial */}
            {!isPremium() && (
        ...
        <Fade in={openPayment}>
          <Box
            sx={{
              ...
              width: { xs: '95%', md: 850 }, // Wider for side-by-side layout
              bgcolor: '#FFFFFF',
              borderRadius: '24px',
